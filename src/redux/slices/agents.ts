import { createSlice, createSelector, type PayloadAction } from '@reduxjs/toolkit';
import type { AppDispatch, RootState } from '../store';

import { getAgentService } from '../../services/AgentService';
import { getRoomService } from '../../services/RoomService';
import type {
  Agent,
  CreateAgentData,
  UpdateAgentData,
  Voice,
  Room,
} from '../../services/types';
import analytics from '../../lib/analytics';

// ==================== State Types ====================

/**
 * Agent room data structure
 */
export interface AgentRoomData {
  agent: Agent;
  dmRoomId: string | null;
  creatorRoomId: string | null;
  loading: boolean;
}

/**
 * Voice state structure
 */
export interface VoicesState {
  items: Voice[];
  loading: boolean;
  error: string | null;
  hasMore: boolean;
  nextPageToken: string | null;
  searchQuery: string;
}

/**
 * Rooms state structure
 */
export interface RoomsState {
  items: Room[];
  loading: boolean;
  error: string | null;
}

/**
 * Agents slice state
 */
export interface AgentsState {
  isLoading: boolean;
  error: string | null;
  agents: Agent[];
  currentAgent: Agent | null;
  currentAgentDmRoomId: string | null;
  currentAgentCreatorRoomId: string | null;
  agentRooms: Record<string, AgentRoomData>;
  voices: VoicesState;
  rooms: RoomsState;
}

// ==================== Initial State ====================

const initialState: AgentsState = {
  isLoading: false,
  error: null,
  agents: [],
  currentAgent: null,
  currentAgentDmRoomId: null,
  currentAgentCreatorRoomId: null,
  agentRooms: {},
  voices: {
    items: [],
    loading: false,
    error: null,
    hasMore: false,
    nextPageToken: null,
    searchQuery: '',
  },
  rooms: {
    items: [],
    loading: false,
    error: null,
  },
};

// ==================== Slice ====================

const slice = createSlice({
  name: 'agents',
  initialState,
  reducers: {
    startLoading(state) {
      state.isLoading = true;
    },

    hasError(state, action: PayloadAction<string>) {
      state.isLoading = false;
      state.error = action.payload;
    },

    stopLoading(state) {
      state.isLoading = false;
    },

    setAgents(state, action: PayloadAction<Agent[]>) {
      state.agents = action.payload;
    },

    setAgent(
      state,
      action: PayloadAction<{
        agent: Agent;
        dmRoomId: string | null;
        creatorRoomId: string | null;
      }>,
    ) {
      state.currentAgent = action.payload.agent;
      state.currentAgentDmRoomId = action.payload.dmRoomId;
      state.currentAgentCreatorRoomId = action.payload.creatorRoomId;
      state.isLoading = false;

      // Also store in agentRooms for multi-agent support
      if (action.payload.agent) {
        state.agentRooms[action.payload.agent.id] = {
          agent: action.payload.agent,
          dmRoomId: action.payload.dmRoomId,
          creatorRoomId: action.payload.creatorRoomId,
          loading: false,
        };
      }
    },

    // New actions for multi-agent support
    startLoadingAgentRoom(state, action: PayloadAction<string>) {
      const agentId = action.payload;
      if (!state.agentRooms[agentId]) {
        state.agentRooms[agentId] = {
          agent: {} as Agent,
          dmRoomId: null,
          creatorRoomId: null,
          loading: true,
        };
      } else {
        state.agentRooms[agentId].loading = true;
      }
    },

    setAgentRoom(
      state,
      action: PayloadAction<{
        agentId: string;
        agent: Agent;
        dmRoomId: string | null;
        creatorRoomId: string | null;
      }>,
    ) {
      const { agentId, agent, dmRoomId, creatorRoomId } = action.payload;
      state.agentRooms[agentId] = {
        agent,
        dmRoomId,
        creatorRoomId,
        loading: false,
      };
    },

    removeAgentRoom(state, action: PayloadAction<string>) {
      const agentId = action.payload;
      delete state.agentRooms[agentId];
    },

    addAgent(state, action: PayloadAction<Agent>) {
      state.agents = [...state.agents, action.payload];
    },

    patchAgent(state, action: PayloadAction<Agent>) {
      const updatedAgent = action.payload;
      state.agents = state.agents.map((agent) =>
        agent.id === updatedAgent.id ? updatedAgent : agent,
      );
      if (state.currentAgent && state.currentAgent.id === updatedAgent.id) {
        state.currentAgent = updatedAgent;
      }
    },

    deleteAgentFromState(state, action: PayloadAction<string>) {
      state.agents = state.agents.filter((agent) => agent.id !== action.payload);
    },

    // Clear state when switching accounts
    clearState(state) {
      Object.assign(state, initialState);
    },

    // Voice actions
    startLoadingVoices(state) {
      state.voices.loading = true;
      state.voices.error = null;
    },

    loadVoicesSuccess(
      state,
      action: PayloadAction<{
        items: Voice[];
        hasMore: boolean;
        nextPageToken: string | null;
        searchQuery: string;
      }>,
    ) {
      const { items, hasMore, nextPageToken, searchQuery } = action.payload;

      let allVoices: Voice[];
      if (searchQuery === state.voices.searchQuery) {
        // Merge and deduplicate
        const combined = [...state.voices.items, ...items];
        const uniqueVoices = combined.reduce((acc: Voice[], voice: Voice) => {
          if (!acc.find((v) => v.voice_id === voice.voice_id)) {
            acc.push(voice);
          }
          return acc;
        }, []);
        allVoices = uniqueVoices;
      } else {
        allVoices = items;
      }

      state.voices.items = allVoices;
      state.voices.hasMore = hasMore;
      state.voices.nextPageToken = nextPageToken;
      state.voices.searchQuery = searchQuery;
      state.voices.loading = false;
    },

    loadVoicesError(state, action: PayloadAction<string>) {
      state.voices.loading = false;
      state.voices.error = action.payload;
    },

    // Room actions
    startLoadingRooms(state) {
      state.rooms.loading = true;
      state.rooms.error = null;
    },

    loadRoomsSuccess(state, action: PayloadAction<Room[]>) {
      state.rooms.items = action.payload;
      state.rooms.loading = false;
    },

    loadRoomsError(state, action: PayloadAction<string>) {
      state.rooms.loading = false;
      state.rooms.error = action.payload;
    },
  },
});

// ==================== Reducer ====================

export default slice.reducer;

// ==================== Thunk Actions ====================

/**
 * Lightweight version that only fetches DM room (for prefetching)
 */
export const fetchAgentDmRoom =
  (agentId: string) =>
  async (dispatch: AppDispatch, getState: () => RootState): Promise<AgentRoomData> => {
    // Check if room is already cached
    const { agentRooms } = getState().agents;
    if (agentRooms[agentId] && agentRooms[agentId].dmRoomId) {
      return Promise.resolve(agentRooms[agentId]);
    }

    dispatch(slice.actions.startLoadingAgentRoom(agentId));

    try {
      const agentService = getAgentService();
      const agentRes = await agentService.fetchAgent(agentId);
      const agent = agentRes.agent;

      if (agent) {
        const { account } = getState().general;

        // Fetch DM room only
        const dmResponse = await agentService.fetchAgentDM(agent.id, account.id);

        const roomData: AgentRoomData = {
          agent,
          dmRoomId: dmResponse.id,
          creatorRoomId: null, // Not fetched in lightweight version
          loading: false,
        };

        // Store in agentRooms
        dispatch(
          slice.actions.setAgentRoom({
            agentId: agent.id,
            agent,
            dmRoomId: dmResponse.id,
            creatorRoomId: null,
          }),
        );

        return Promise.resolve(roomData);
      } else {
        throw new Error('Agent not found');
      }
    } catch (error) {
      return Promise.reject(error);
    }
  };

/**
 * Fetch agent room with DM and creator room
 */
export const fetchAgentRoom =
  (agentId: string, setAsCurrent = true) =>
  async (dispatch: AppDispatch, getState: () => RootState): Promise<AgentRoomData> => {
    // Check if room is already cached
    const { agentRooms } = getState().agents;
    if (agentRooms[agentId] && agentRooms[agentId].dmRoomId) {
      // Already loaded, just set as current if requested
      if (setAsCurrent) {
        const roomData = agentRooms[agentId];
        dispatch(
          slice.actions.setAgent({
            agent: roomData.agent,
            dmRoomId: roomData.dmRoomId,
            creatorRoomId: roomData.creatorRoomId,
          }),
        );
      }
      return Promise.resolve(agentRooms[agentId]);
    }

    if (setAsCurrent) {
      dispatch(slice.actions.startLoading());
    }
    dispatch(slice.actions.startLoadingAgentRoom(agentId));

    try {
      const agentService = getAgentService();
      const roomService = getRoomService();

      const agentRes = await agentService.fetchAgent(agentId);
      const agent = agentRes.agent;

      if (agent) {
        const { account } = getState().general;

        // Fetch DM room
        const dmResponse = await agentService.fetchAgentDM(agent.id, account.id);

        // Fetch creator room - some agents don't have a creator room, so handle 404 gracefully
        let creatorRoomId: string | null = null;
        try {
          const creatorResponse = await roomService.fetchRoomByExternalId(
            `agent_${agent.id}`,
            agent.account_id,
          );
          creatorRoomId = creatorResponse.room.id;
        } catch {
          // If creator room doesn't exist (404) or any other error, just continue without it
          // creatorRoomId will remain null
        }

        const roomData: AgentRoomData = {
          agent,
          dmRoomId: dmResponse.id,
          creatorRoomId,
          loading: false,
        };

        // Store in agentRooms
        dispatch(
          slice.actions.setAgentRoom({
            agentId: agent.id,
            agent,
            dmRoomId: dmResponse.id,
            creatorRoomId,
          }),
        );

        // Also set as current if requested (for backwards compatibility)
        if (setAsCurrent) {
          dispatch(
            slice.actions.setAgent({
              agent,
              dmRoomId: dmResponse.id,
              creatorRoomId,
            }),
          );
        }

        return Promise.resolve(roomData);
      } else {
        throw new Error('Agent not found');
      }
    } catch (error) {
      if (setAsCurrent) {
        dispatch(slice.actions.hasError((error as Error).toString()));
      }
      return Promise.reject(error);
    }
  };

/**
 * Create a new agent
 */
export const createAgent =
  (data: CreateAgentData) =>
  async (dispatch: AppDispatch, getState: () => RootState): Promise<Agent> => {
    dispatch(slice.actions.startLoading());
    const { account } = getState().general;
    try {
      const agentService = getAgentService();
      const response = await agentService.createAgent(account.id, data);
      const { agent } = response;
      dispatch(slice.actions.addAgent(agent));

      // Track agent creation
      analytics.track('created_agent', {
        agent_id: agent.id,
        agent_type: data.meta_data?.agent_type || 'General Assistant',
        goal: data.meta_data?.goal || null,
        industry: data.meta_data?.industry || null,
        has_voice: !!data.voice,
        created_from: data.meta_data?.created_from || 'unknown',
      });

      return agent;
    } catch (e) {
      dispatch(slice.actions.hasError((e as Error).toString()));
      return Promise.reject((e as Error).toString());
    } finally {
      dispatch(slice.actions.stopLoading());
    }
  };

/**
 * Update an existing agent
 */
export const updateAgent =
  (agentId: string, data: UpdateAgentData) =>
  async (dispatch: AppDispatch): Promise<Agent> => {
    try {
      const agentService = getAgentService();
      const response = await agentService.updateAgent(agentId, data);
      const { agent } = response;
      dispatch(slice.actions.patchAgent(agent));

      // Track agent update
      analytics.track('updated_agent', {
        agent_id: agentId,
        updated_fields: Object.keys(data),
      });

      return Promise.resolve(agent);
    } catch (e) {
      const error = e as { response?: { data?: { detail?: string } }; message?: string };
      if (error.response && error.response.data && error.response.data.detail) {
        return Promise.reject(error.response.data.detail);
      } else {
        return Promise.reject(error.message);
      }
    }
  };

/**
 * Delete an agent
 */
export const deleteAgent =
  (agentId: string) =>
  async (dispatch: AppDispatch): Promise<void> => {
    try {
      const agentService = getAgentService();
      await agentService.deleteAgent(agentId);
      dispatch(slice.actions.deleteAgentFromState(agentId));
      return Promise.resolve();
    } catch (e) {
      return Promise.reject(e);
    }
  };

/**
 * Fetch available voices
 */
export const fetchVoices =
  (search = '', loadMore = false) =>
  async (dispatch: AppDispatch, getState: () => RootState): Promise<void> => {
    const { voices } = getState().agents;

    if (!loadMore) {
      dispatch(slice.actions.startLoadingVoices());
    }

    try {
      const agentService = getAgentService();
      const options: { search?: string; next_page_token?: string } = {};
      if (search) options.search = search;
      if (loadMore && voices.nextPageToken) {
        options.next_page_token = voices.nextPageToken;
      }

      const response = await agentService.listVoices(options);

      dispatch(
        slice.actions.loadVoicesSuccess({
          items: response.voices,
          hasMore: response.has_more,
          nextPageToken: response.next_page_token || null,
          searchQuery: search,
        }),
      );
    } catch (error) {
      const err = error as { response?: { data?: { detail?: string } }; message?: string };
      const errorMessage = err.response?.data?.detail || err.message || 'Unknown error';
      dispatch(slice.actions.loadVoicesError(errorMessage));
      throw errorMessage;
    }
  };

/**
 * Fetch rooms associated with an agent
 */
export const fetchAgentRooms =
  (agentId: string) =>
  async (dispatch: AppDispatch): Promise<void> => {
    dispatch(slice.actions.startLoadingRooms());
    try {
      const agentService = getAgentService();
      const response = await agentService.fetchAgentRooms(agentId);
      dispatch(slice.actions.loadRoomsSuccess(response.rooms));
    } catch (error) {
      const err = error as { response?: { data?: { detail?: string } }; message?: string };
      const errorMessage = err.response?.data?.detail || err.message || 'Unknown error';
      dispatch(slice.actions.loadRoomsError(errorMessage));
      throw errorMessage;
    }
  };

/**
 * Fetch agent by ID
 */
export const fetchAgentById =
  (agentId: string) =>
  async (dispatch: AppDispatch, getState: () => RootState): Promise<Agent> => {
    try {
      // Check if agent already exists in agents store
      const { agents } = getState().agents;
      const existingAgent = agents?.find((a) => a.id === agentId);
      if (existingAgent) {
        return Promise.resolve(existingAgent);
      }

      // Also check general store (legacy support)
      const { account } = getState().general;
      const existingInGeneral = account?.agents?.find((a: Agent) => a.id === agentId);
      if (existingInGeneral) {
        // Add to agents store for consistency
        dispatch(slice.actions.addAgent(existingInGeneral));
        return Promise.resolve(existingInGeneral);
      }

      // Fetch the agent
      const agentService = getAgentService();
      const response = await agentService.fetchAgent(agentId);
      const { agent } = response;

      // Add to agents store
      dispatch(slice.actions.addAgent(agent));

      return Promise.resolve(agent);
    } catch (e) {
      const error = e as { response?: { data?: { detail?: string } }; message?: string };
      const errorMessage = error.response?.data?.detail || error.message || 'Unknown error';
      // eslint-disable-next-line no-console
      console.error(`Error fetching agent ${agentId}:`, errorMessage);
      return Promise.reject(errorMessage);
    }
  };

// ==================== Exported Actions ====================

export const { setAgents, removeAgentRoom, clearState: clearAgentsState } = slice.actions;

// ==================== Selectors ====================

// Base selectors
const selectAgentsFromAgentsStore = (state: RootState): Agent[] =>
  state.agents.agents || [];
const selectAgentsFromGeneralStore = (state: RootState): Agent[] =>
  (state.general.account?.agents as Agent[]) || [];

// Memoized selector to prevent unnecessary re-renders
export const selectAllAgents = createSelector(
  [selectAgentsFromAgentsStore, selectAgentsFromGeneralStore],
  (agentsFromAgentsStore, agentsFromGeneralStore): Agent[] => {
    // Merge both stores, preferring agents store (for fetched agents)
    return [
      ...agentsFromAgentsStore,
      ...agentsFromGeneralStore.filter(
        (a) => !agentsFromAgentsStore.find((agent) => agent.id === a.id),
      ),
    ];
  },
);

// Selector for getting a specific agent's room data
export const selectAgentRoom = (agentId: string) => (state: RootState): AgentRoomData | null => {
  return state.agents.agentRooms[agentId] || null;
};

